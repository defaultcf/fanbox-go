...
func (s PostBodyBlocksItemStylesItemType) Validate() error {
	switch s {
	case "bold":
		return nil
	default:
...
func (s *UpdatePostReq) Validate() error {
	...
	if err := func() error {
		if value, ok := s.CommentingPermissionScope.Get(); ok {
			if err := func() error {
				if err := value.Validate(); err != nil {
					return err
				}
...
func (s UpdatePostReqCommentingPermissionScope) Validate() error {
	switch s {
	case "everyone":
		return nil
	case "supporters":
